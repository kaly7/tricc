<?php
// Shared renderer: turns a template + partner + project into filled HTML fragment
...
function render_template_html(PDO $pdo, int $template_id, ?int $partner_id, ?int $project_id, string $defaultImgW='40mm'): string {
  // Template
  $stmt = $pdo->prepare("SELECT * FROM templates WHERE id=:id");
  $stmt->execute([':id'=>$template_id]);
  $tpl = $stmt->fetch(PDO::FETCH_ASSOC);
  if (!$tpl) return '<div class="err">Sablon nem található.</div>';
  $html = (string)$tpl['content_html'];

  // Partner (fallback first)
  ...
  foreach($map as $k=>$v){ $html = str_replace('{{ '.$k.' }}', htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8'), $html); }

  // Contacts default + indexed
  ...
  // Images: {{ image.key }} or with options |w=..|h=..
  $allowedUnits=['mm','px','cm'];
  $base = base_url();
  $html = preg_replace_callback('/\{\{\s*image\.([a-z0-9_\-]+)(\|[^}]*)?\s*\}\}/i', function($m) use ($images, $defaultImgW, $allowedUnits, $base){
    $key=$m[1]; $optstr = isset($m[2]) ? $m[2] : '';
    $opts=[];
    if ($optstr){
      $optstr=ltrim($optstr,'|');
      foreach(explode('|',$optstr) as $chunk){
        ...
        $kv=explode('=',$chunk,2);
        if(count($kv)==2){ $opts[strtolower(trim($kv[0]))]=trim($kv[1]); }
      }
    }
    if (!isset($images[$key])){
    ...
    $img=$images[$key];
    // Absolute URL so any renderer / archives/* find it
    $src= rtrim($base,'/') . '/uploads/' . rawurlencode($img['stored_name']);
    ...
    $wcss=$sanitize($w,'40mm'); $hcss=$sanitize($h,'auto');
    $style = 'max-width:'.$wcss.'; height:auto;';
    if ($hcss !== 'auto'){
      $style = 'width:'.$wcss.'; height:'.$hcss.'; object-fit:contain;';
    }
