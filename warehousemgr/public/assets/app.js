document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateCalculated() {
    ...
    if (mode === 'end') {
      ...
      const total = toMinutes(eh, em) - toMinutes(sh, sm) - br;
      if (calcDuration) calcDuration.textContent = total >= 0 ? human(total) : 'Érvénytelen';
    } else {
      ...
      if (calcEnd) calcEnd.textContent = `${pad(Math.floor((endTotal / 60) % 24))}:${pad(endTotal % 60)}`;
    }
  }
