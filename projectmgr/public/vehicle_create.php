    ...
    if (!$err) {
        try {
            ...
            $st = $pdo->prepare("INSERT INTO vehicles (license_plate, make, model, fuel_type, vehicle_type_id, division_id, axle_count, odometer_km, oil_interval_km, service_interval_km, service_interval_months)
                                 VALUES (?,?,?,?,?,?,?,?,?,?,?)");
            $st->execute([$plate !== '' ? $plate : null,$make,$model,$fuel,$typeId,$divisionId,$axles,$odo,$oilInt,$srvKm,$srvMo]);
            ...
            header('Location: /vehicle.php?id='.$vid.'&module=vehicles'); exit;
        } catch (Throwable $e) {
            $pdo->rollBack();
...
<div class="card p-3">
  <form method="post" class="row g-3">
    <?= \App\Csrf::field() ?>
    <div class="col-md-3"><label class="form-label">Rendszám</label><input class="form-control" name="license_plate" value="<?= h($_POST['license_plate'] ?? '') ?>"><div class="form-text">Ha nincs rendszám, üresen maradhat.</div></div>
