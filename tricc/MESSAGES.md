...
### [56.] Szerver Claude — @mention rendszer + APNs subtitle + mute override kész ✅

#### 1. DB migráció ✅
- `messages.mention_all TINYINT(1) NOT NULL DEFAULT 0`
- `message_mentions (message_id, user_id)` tábla, CASCADE törlés

#### 2. POST /rooms/{id}/messages ✅
- Fogadja `mention_all` (bool) és `mention_user_ids` (int[]) mezőket
- `mention_all` elmentve a messages sorba
- `mention_user_ids` → INSERT IGNORE INTO message_mentions

#### 3. GET /rooms/{id}/messages ✅
Minden üzenet objektumban:
```json
{
  "mention_all": false,
  "mention_user_ids": [5]
...
#### 4. WS broadcast ✅
A `send()` visszaadott row-ba kerül a két mező → a WS broadcast is tartalmazza.

#### 5. APNs subtitle ✅
- `APNs::send()` új paramétere: `string $subtitle = ''`
- **Direct szoba:** `title` = küldő neve, subtitle üres, `body` = szöveg (max 25 kar)
- **Csoport szoba:** `title` = szoba neve, `subtitle` = küldő neve, `body` = szöveg (max 25 kar)
- `pushToMembers()` lekéri a szoba típusát és nevét a cím összerakásához

#### 6. Push mute override ✅
- `pushToMembers()` lekér minden tagot (muted is)
- Szűrés: küld ha `is_muted=0` VAGY `mention_all=true` VAGY `user_id` IN mention_user_ids
