...
class TelemetryService
{
    ...
    public function store(string $deviceId, array $payload): void
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO telemetry_log (device_id, ts, temperature, humidity, air_quality, battery_pct, power_mode, contact_1, contact_2, contact_3, contact_4, rssi, raw_json) VALUES (:device_id, :ts, :temperature, :humidity, :air_quality, :battery_pct, :power_mode, :contact_1, :contact_2, :contact_3, :contact_4, :rssi, :raw_json)");
        $stmt->execute([
    ...
    private function upsertLastState(string $deviceId, array $payload, int $online = 1, ?string $serverNow = null): void
    {
        ...
        $upsert = $this->db->prepare("INSERT INTO device_last_state (device_id, last_seen_at, online, power_mode, battery_pct, temperature, humidity, air_quality, contact_1, contact_2, contact_3, contact_4, rssi, reported_config_version, raw_json, updated_at)
        VALUES (:device_id, :last_seen_at, :online, :power_mode, :battery_pct, :temperature, :humidity, :air_quality, :contact_1, :contact_2, :contact_3, :contact_4, :rssi, :reported_config_version, :raw_json, NOW())
        ON DUPLICATE KEY UPDATE
            ...
            temperature = COALESCE(VALUES(temperature), temperature), humidity = COALESCE(VALUES(humidity), humidity), air_quality = COALESCE(VALUES(air_quality), air_quality),
    ...
    public function historySeries(string $deviceId, \DateTimeImmutable $from, \DateTimeImmutable $to, int $targetPoints = 720): array
    {
        ...
        $stmt = $this->db->prepare("SELECT ts, temperature, humidity, contact_1, contact_2, contact_3, contact_4, rssi, raw_json FROM telemetry_log WHERE device_id = :device_id AND ts BETWEEN :from_ts AND :to_ts ORDER BY ts ASC");
        $stmt->execute([
            'device_id' => $deviceId,
