...
$id = (int)($_GET['id'] ?? 0);
$st = $pdo->prepare("SELECT * FROM pbx_devices WHERE id=?");
$st->execute([$id]);
$d = $st->fetch();
if (!$d) { http_response_code(404); exit('Nincs ilyen mellék'); }

$pbx_id = (int)$d['pbx_id'];

$pbxSt = $pdo->prepare("SELECT id,name FROM pbx_systems WHERE id=?");
$pbxSt->execute([$pbx_id]);
$pbx = $pbxSt->fetch();
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($extension==='') $err='Mellékszám kötelező.';
    ...
    $up = $pdo->prepare("UPDATE pbx_devices SET catalog_item_id=?, extension=?, ip=?, access_url=?, access_user=?, access_pass=?, admin_url=?, reboot_url=?, notes=?, is_archived=? WHERE id=?");
    $up->execute([$catalog_item_id, $extension, $ip ?: null, $access_url ?: null, $access_user ?: null, $access_pass ?: null, $admin_url ?: null, $reboot_url ?: null, $notes ?: null, $is_archived, $id]);
    flash_set('ok', 'Mentve.');
    redirect('pbx_system_show.php?id='.$pbx_id);
  }
}

$title='Mellék szerkesztése';
$page='Központok';
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h1 class="h3 mb-1">Mellék szerkesztése</h1>
...
<div class="card p-3">
  <form method="post" class="row g-3">
    ...
    <div class="col-12 col-md-3">
      ...
      <input class="form-control" name="extension" value="<?= e($d['extension']) ?>" required>
    </div>
        ...
        <div class="col-12 col-md-6">
          <select class="form-select" name="catalog_item_id">
            ...
            <?php foreach ($types as $t): ?>
              <option value="<?= (int)$t['id'] ?>" <?= (int)$d['catalog_item_id']===(int)$t['id']?'selected':'' ?>><?= e($t['manufacturer_name'].' / '.$t['model']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-12 col-md-3">
    ...
    <div class="col-12 col-md-3">
      ...
      <input class="form-control" name="ip" value="<?= e($d['ip'] ?? '') ?>">
    </div>
    <div class="col-12 col-md-6">
      <label class="form-label">URL</label>
      <input class="form-control" name="access_url" value="<?= e($d['access_url'] ?? '') ?>">
      <div class="form-text">Alap elérés (pl. https://10.0.0.12)</div>
    </div>
    <div class="col-12 col-md-3">
      <label class="form-label" for="admin_url">Admin URL (felülírás)</label>
      <input id="admin_url" class="form-control" type="text" name="admin_url"
             value="<?= e((string)($d['admin_url'] ?? '')) ?>"
             placeholder="pl. admin vagy #/advanced">
      <div class="form-text">Ha üres, az eszköztípus alapértelmezett admin kiegészítése lesz használva.</div>
    </div>

    <div class="col-12 col-md-3">
      <label class="form-label" for="reboot_url">Reboot URL (felülírás)</label>
      <input id="reboot_url" class="form-control" type="text" name="reboot_url"
             value="<?= e((string)($d['reboot_url'] ?? '')) ?>"
             placeholder="pl. reboot vagy action/restart">
      <div class="form-text">Ha üres, az eszköztípus alapértelmezett reboot kiegészítése lesz használva.</div>
    </div>
    <div class="col-12 col-md-3">
      <label class="form-label">Felhasználó</label>
      <input class="form-control" name="access_user" value="<?= e($d['access_user'] ?? '') ?>">
    </div>
    <div class="col-12 col-md-3">
      <label class="form-label">Jelszó</label>
      <input class="form-control" name="access_pass" value="<?= e($d['access_pass'] ?? '') ?>">
    </div>

    <div class="col-12">
      <label class="form-label">Megjegyzés</label>
      <textarea class="form-control" name="notes" rows="4"><?= e($d['notes'] ?? '') ?></textarea>
    </div>

    <div class="col-12">
      <div class="form-check">
        <input class="form-check-input" type="checkbox" name="is_archived" id="arch" value="1" <?= (int)$d['is_archived']===1?'checked':'' ?>>
