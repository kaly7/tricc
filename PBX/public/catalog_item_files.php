...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_FILES['files'])) {
  ...
  $dir = dirname(__DIR__).'/storage/catalog_files/'.$id;
  ...
  for($i=0;$i<$count;$i++){
    ...
    // Center auth: don't rely on local users FK
    $ins = $pdo->prepare("INSERT INTO catalog_files
      (catalog_item_id, original_name, stored_name, mime, size, uploaded_by, uploaded_by_name, uploaded_by_email, created_at, is_deleted)
      VALUES
      (:cid,:orig,:stored,:mime,:size,NULL,:uname,:uemail,NOW(),0)");
    $ins->execute([
      ':cid'=>$id,
      ':orig'=>$orig,
      ':stored'=>$stored,
  ...
  header('Location: '.base_url('catalog_item_files.php?id='.$id));
  exit;
}

// List
$list = $pdo->prepare("SELECT * FROM catalog_files WHERE catalog_item_id=? AND is_deleted=0 ORDER BY created_at DESC, id DESC");
$list->execute([$id]);
$rows = $list->fetchAll();
...
$label = trim(($item['manufacturer_name'] ?? '').' '.($item['model'] ?? ''));
?>
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h1 class="h3 mb-1">Dokumentáció</h1>
    <div class="text-muted small"><?= e($label ?: ('#'.$id)) ?></div>
  </div>
  <div class="d-flex gap-2 flex-wrap">
    <a class="btn btn-outline-secondary" href="<?= e(base_url('catalog_item_edit.php?id='.$id)) ?>">Vissza</a>
  </div>
</div>
          ...
          <tbody>
            <?php foreach($rows as $r): ?>
              <tr>
                ...
                <td class="text-end">
                  <a class="btn btn-sm btn-outline-primary" href="<?= e(base_url('catalog_file_download.php?id='.(int)$r['id'])) ?>">Letöltés</a>
                  <?php if (current_user() && in_array((string)(current_user()['role'] ?? ''), ['admin','editor'], true)): ?>
                    <a class="btn btn-sm btn-outline-danger"
                       href="<?= e(base_url('catalog_file_delete.php?id='.(int)$r['id'].'&cid='.$id)) ?>"
