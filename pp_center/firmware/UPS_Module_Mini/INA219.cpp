...
{
    uint16_t current;
    current = INA219_wireReadRegister(INA219_REG_CONFIG);
    uint8_t next;
    if (on) {
...
{
    uint16_t value;
    value = INA219_wireReadRegister(INA219_REG_SHUNTVOLTAGE);
...
{
    uint16_t value = 0;
    value = INA219_wireReadRegister(INA219_REG_BUSVOLTAGE);
...
float INA219_getCurrent_mA() 
{
  ...
  value = INA219_wireReadRegister(INA219_REG_CURRENT);
...
float INA219_getPower_mW() {
  ...
  value = INA219_wireReadRegister(INA219_REG_POWER);
...
/*!
 *  @brief  Getting the charging status
 *  @return 
 */
