...
class MessageController {
    ...
    public static function send(int $room_id): never {
        ...
        $mention_all      = (bool)($body['mention_all'] ?? false);
        $mention_user_ids = array_map('intval', $body['mention_user_ids'] ?? []);
        ...
        $db->prepare("INSERT INTO messages (room_id, sender_id, type, content, mention_all, file_url, file_name, file_size, reply_to_id, reply_to_content, reply_to_user_name) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
           ->execute([$room_id, $auth['user_id'], $type, $content ?: '', $mention_all ? 1 : 0, $file_url ?? '', $file_name, $file_size, $reply_to_id ?: null, $reply_to_content, $reply_to_user_name]);
        $msg_id = (int)$db->lastInsertId();

        foreach ($mention_user_ids as $uid) {
            $db->prepare("INSERT IGNORE INTO message_mentions (message_id, user_id) VALUES (?,?)")
               ->execute([$msg_id, $uid]);
        }
        ...
        $row['deliveries']       = [];
        $row['reactions']        = [];
        $row['mention_all']      = $mention_all;
        $row['mention_user_ids'] = $mention_user_ids;
        self::pushToMembers($room_id, $auth['user_id'], $row, $msg_id, $mention_all, $mention_user_ids);
    ...
    public static function edit(int $room_id, int $msg_id): never {
        ...
        $ms = $db->prepare("SELECT user_id FROM message_mentions WHERE message_id=?");
        $ms->execute([$msg_id]);
        $updated['mention_user_ids'] = array_map('intval', array_column($ms->fetchAll(), 'user_id'));
        $updated['mention_all']      = (bool)$updated['mention_all'];
        $updated['deliveries'] = [];
        $updated['reactions']  = [];
    ...
    private static function pushToMembers(int $room_id, int $sender_id, array $msg, int $msg_id, bool $mention_all = false, array $mention_user_ids = []): void {
        $db = DB::get();
        $sender = $db->prepare("SELECT name FROM users WHERE id=?");
        $sender->execute([$sender_id]);
        $sname = $sender->fetchColumn() ?? 'Ismeretlen';

        $room = $db->prepare("SELECT type, name FROM rooms WHERE id=?");
        $room->execute([$room_id]);
        $roomRow = $room->fetch();

        if ($roomRow && $roomRow['type'] === 'group') {
            $title    = mb_substr($roomRow['name'], 0, 25, 'UTF-8');
            $subtitle = mb_substr($sname, 0, 25, 'UTF-8');
        } else {
            $title    = mb_substr($sname, 0, 25, 'UTF-8');
            $subtitle = '';
        }
        ...
        $tokenRows = array_filter($tokens->fetchAll(), function ($t) use ($mention_all, $mention_user_ids) {
            if (!$t['is_muted']) return true;
            if ($mention_all)    return true;
            return in_array((int)$t['user_id'], $mention_user_ids);
        });
