...
if (($_POST['action'] ?? '') === 'create') {
  try {
    ...
    if(!preg_match('/^[a-z0-9_]{2,64}$/',$name)) throw new Exception('Mező azonosító csak kisbetű/szám/alsóvonás (2-64).');
    ...
    $stmt = $pdo->prepare("INSERT INTO partner_fields (name,label,type,options_json,required,sort_order,active) VALUES (:name,:label,:type,:opts,:req,:sort,1)");
    $stmt->execute([':name'=>$name, ':label'=>$label, ':type'=>$type, ':opts'=>$options, ':req'=>$required, ':sort'=>$sort]);
    $ok="Mező létrehozva.";
...
if (($_POST['action'] ?? '') === 'toggle') {
  ...
  $pdo->prepare("UPDATE partner_fields SET active=:a WHERE id=:id")->execute([':a'=>$active, ':id'=>$id]);
  $ok="Állapot módosítva.";
}
if (($_POST['action'] ?? '') === 'delete') {
  $id = (int)($_POST['id'] ?? 0);
  $pdo->prepare("DELETE FROM partner_fields WHERE id=:id")->execute([':id'=>$id]);
  $ok="Mező törölve.";
}

$fields = $pdo->query("SELECT * FROM partner_fields ORDER BY sort_order ASC, id ASC")->fetchAll();
?>
  <div class="container">
    <div class="hdr card">
      <div><strong>Partner mezők</strong></div>
    ...
    <div class="card">
      ...
      <form method="post" class="grid cols-3">
        ...
        <div class="input"><label>Azonosító (pl. adoszam)</label><input name="name" required></div>
