...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($action==='create') {
    ...
    $hex = trim((string)($_POST['hex_code'] ?? '')); $hex = ($hex==='') ? null : $hex;
    if ($name!=='') {
      $pdo->prepare("INSERT INTO {$table} (name, hex_code) VALUES (?, ?)")->execute([$name, $hex]);
  ...
  header('Location: /vehicle_colors.php?module=vehicles');
  exit;
}
...
<div class="card p-3 mb-3">
  <form method="post" class="row g-2">
    ...
    <div class="col-md-3"><label class="form-label">HEX</label><input class="form-control" name="hex_code" placeholder="#RRGGBB"></div>
    <div class="col-12">
    ...
    <tbody>
      <?php foreach($rows as $r): ?>
        <tr>
          ...
          <td><?php $hx=(string)($r['hex_code']??''); ?><?= htmlspecialchars($hx) ?><?php if($hx!==''): ?> <span style="display:inline-block;width:14px;height:14px;border:1px solid #ccc;vertical-align:-2px;background:<?= htmlspecialchars($hx) ?>"></span><?php endif; ?></td>
